...
function multiply(num1, num2, callback3){
  callback3(num1 * num2);
}
...
function first(names, callback4) {
  callback4(names.splice(0,1))
}
...
function last(names, callback5) {
  callback5(names.splice(names.length - 1, 1))
}
...
function contains(names, nameCheck, callback6) {
  for (let i = 0; i < names.length; i++){
    if (nameCheck === names[i]){
      return callback6(true);
    }
    else {
